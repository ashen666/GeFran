...
public class GosControlModuleBaseActivity extends GosBaseActivity {
	 ...
	 */
	// 数据点"手/自"对应的标识名
	protected static final String KEY_MANUAL_AUTO = "Manual_Auto";
	// 数据点"制冷"对应的标识名
	protected static final String KEY_REFRIGERATION = "Refrigeration";
	// 数据点"制热"对应的标识名
	protected static final String KEY_HEATING = "Heating";
	// 数据点"除湿"对应的标识名
	protected static final String KEY_AREFACTION = "Arefaction";
	// 数据点"加湿"对应的标识名
	protected static final String KEY_HUMIDIFICATION = "Humidification";
	// 数据点"状态"对应的标识名
	protected static final String KEY_MANUAL_AUTO_STATUS = "Manual_Auto_status";
	// 数据点"湿机"对应的标识名
	protected static final String KEY_DEHUMIDIFICATION = "Dehumidification";
	// 数据点"空调"对应的标识名
	protected static final String KEY_AIRCONDITIONING = "AirConditioning";
	// 数据点"湿度"对应的标识名
	protected static final String KEY_HUMIDITY = "Humidity";
	// 数据点"温度"对应的标识名
	protected static final String KEY_TEMPERATURE = "Temperature";
	// 数据点"温度报警"对应的标识名
	protected static final String KEY_TEMPERATURE_ALARM = "Temperature_alarm";
	// 数据点"湿度报警"对应的标识名
	protected static final String KEY_HUMIDITY_ALARM = "Humidity_alarm";
	 ...
	 */
	// 数据点"手/自"对应的存储数据
	protected static boolean data_Manual_Auto;
	// 数据点"制冷"对应的存储数据
	protected static boolean data_Refrigeration;
	// 数据点"制热"对应的存储数据
	protected static boolean data_Heating;
	// 数据点"除湿"对应的存储数据
	protected static boolean data_Arefaction;
	// 数据点"加湿"对应的存储数据
	protected static boolean data_Humidification;
	// 数据点"状态"对应的存储数据
	protected static int data_Manual_Auto_status;
	// 数据点"湿机"对应的存储数据
	protected static int data_Dehumidification;
	// 数据点"空调"对应的存储数据
	protected static int data_AirConditioning;
	// 数据点"湿度"对应的存储数据
	protected static double data_Humidity;
	// 数据点"温度"对应的存储数据
	protected static double data_Temperature;
	// 数据点"温度报警"对应的存储数据
	protected static boolean data_Temperature_alarm;
	// 数据点"湿度报警"对应的存储数据
	protected static boolean data_Humidity_alarm;
	...
	protected void getDataFromReceiveDataMap(ConcurrentHashMap<String, Object> dataMap) {
		...
		if (dataMap.get("data") != null) {
			...
			for (String dataKey : map.keySet()) {
				if (dataKey.equals(KEY_MANUAL_AUTO)) {
					data_Manual_Auto = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_REFRIGERATION)) {
					data_Refrigeration = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_HEATING)) {
					data_Heating = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_AREFACTION)) {
					data_Arefaction = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_HUMIDIFICATION)) {
					data_Humidification = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_MANUAL_AUTO_STATUS)) {
					data_Manual_Auto_status = (Integer) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_DEHUMIDIFICATION)) {
					data_Dehumidification = (Integer) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_AIRCONDITIONING)) {
					data_AirConditioning = (Integer) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_HUMIDITY)) {
					...
					if (map.get(dataKey) instanceof Integer) {
						...
						data_Humidity = (Double) map.get(dataKey);
					}
				}
				if (dataKey.equals(KEY_TEMPERATURE)) {
					...
					if (map.get(dataKey) instanceof Integer) {
						...
						data_Temperature = (Double) map.get(dataKey);
					}
				}
				if (dataKey.equals(KEY_TEMPERATURE_ALARM)) {
					data_Temperature_alarm = (Boolean) map.get(dataKey);
				}
				if (dataKey.equals(KEY_HUMIDITY_ALARM)) {
					data_Humidity_alarm = (Boolean) map.get(dataKey);			
				}
			}
		}
		...
		if (dataMap.get("alerts") != null) {
			...
			for (String alertsKey : map.keySet()) {
				...
				switch (alertsKey) {
					case KEY_TEMPERATURE_ALARM:
						sBuilder.append("温度" + "超过设定值" + "\n");
						Log.i("mm","x:");
						break;
					case KEY_HUMIDITY_ALARM:sBuilder.append("湿度"+ "超过设定值" + "\n");
						Log.i("mm","x:");
						break;
				}
		...
		if (dataMap.get("faults") != null) {
			...
			for (String faultsKey : map.keySet()) {
				if ((Boolean) map.get(faultsKey)) {
					sBuilder.append("故障:" + faultsKey + "过高" + "\n");
				}
			}
		}
